...
export default function ClaimScreen() {
  const { id, foodType, quantity, location, pickupBy, tags } = useLocalSearchParams<{
    id?: string;
    foodType?: string;
    quantity?: string;
    location?: string;
  ...
  useEffect(() => {
    // Reset the success state when a different item is clicked
    setClaimed(false);
    setPickupCode('');
    setClaiming(false);
  }, [id]);
    ...
    <View style={styles.container}>
      ...
      {id && (
        <View style={styles.card}>
          <Text style={styles.title}>{foodType || 'Food'}</Text>
          <Text style={styles.detail}>{quantity || 'Some items'}</Text>
          <Text style={styles.detail}>{location || 'Unknown Location'}</Text>
          <Text style={styles.detail}>Pickup by {pickupBy || 'N/A'}</Text>
      ...
      <Text style={[styles.heading, { marginTop: id ? 24 : 0, fontSize: 20 }]}>Recently Claimed Feed</Text>
      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          <View style={styles.historyCard}>
            <View style={styles.historyTopRow}>
              <Text style={styles.historyTitle}>{item.category || 'Food'}</Text>
              <View style={styles.claimedBadge}>
                <Text style={styles.claimedText}>Claimed</Text>
              </View>
            </View>
            <Text style={styles.historyDetail}>📍 {item.location || 'Unknown Location'}</Text>
            {item.claimedAt && (
              <Text style={styles.historyTime}>
                Claimed at {new Date(item.claimedAt.toMillis()).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
              </Text>
            )}
          </View>
        )}
        ...
        ListEmptyComponent={
          <View style={styles.emptyContainer}>
            ...
            <Text style={styles.emptySubtext}>Be the first to claim food and reduce waste!</Text>
          </View>
        }
      />
    </View>
