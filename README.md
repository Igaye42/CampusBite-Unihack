...
* **Example flow:** 1. Provider snaps a photo.
  2. App sends to Gemini.
  3. API invisibly returns: `{"type": "sandwich", "estimated_qty": 5, "safety_risk": false}`.
...
* **Example flow:** 1. Provider reviews "Sandwich, Qty: 5".
  2. Taps "Vegetarian" toggle.
  3. Taps "Post Listing". App writes a single document to Firestore `listings`.
...
* **Example flow:** 1. Consumer opens feed and sets preference to "Vegetarian".
  2. Feed hides meat items and displays: "Sandwich — 200m away — Expires in 1h 45m".
...
* **Example flow:** 1. Consumer taps "Claim".
  2. Firestore transaction sets status to `claimed`.
  3. App displays: "Claim Successful. Show code: X7B2". Item disappears from other feeds.
...
* **Example flow:** 1. Consumer claims an item.
  2. App silently writes `+1 meal saved` to the database.
  3. Dashboard updates: "Total Meals Rescued: 125".
